...
### Added
- **Multi-provider freedom — epic #101 (A–D)** — the engine's core differentiator, delivered as four slices:
  - **OpenRouter honesty** (#967 via #971) — non-Anthropic model ids that can't take `thinking` now log a one-time WARNING instead of silently dropping it (attempt-mode rejected: accepting models would bill silently); costs computed from fallback rates carry `metadata["cost_estimated"]=True` plus a one-time WARNING; 5 new pricing keys.
  - **Local models via `--base-url`** (#968 via #972) — `orch run --mode openrouter --base-url http://localhost:11434/v1` drives a pipeline against Ollama / LM Studio / vLLM (any OpenAI-compatible endpoint) with **no API key**: the factory relaxes the key requirement iff `base_url` differs from the OpenRouter default (cloud guard byte-identical), sending a defensive `Bearer ollama` placeholder.
  - **Per-phase provider targeting** (#969 via #973) — a template phase may declare `provider: anthropic | openrouter` and one run routes each phase to its provider's executor (opus for the adversary, a local llama for recon, same pipeline). `PipelineRunner.from_providers` builds the multi-executor set eagerly (missing credentials fail fast, naming the env var); unknown providers are a validation **error**; `--api-key` feeds Anthropic only.
  - **`orch providers list`** (#970) — read-only discoverability for all six providers: per-phase support, credential env var + presence (`set`/`missing` booleans — the raw value is **never** printed), default tier maps, maturity. `--json` emits a stable 8-key schema; the command makes zero network calls and constructs no database. Drift guards bind the registry to the live `KNOWN_PROVIDERS` constant and the docs maturity table.
- `orch admin prune-test-runs` (#981) — operator cleanup for pytest-residue `pipeline_runs` (`template_id='hello-pipeline' AND output_dir LIKE '%/.wt/%'`). Dry-run by default (prints the count, deletes nothing); `--no-dry-run --yes` executes and audit-logs the deletion. Never auto-deletes; spares the operator's real non-`.wt` runs.
- Per-phase model-tier BOUNDS: `PhaseDefinition.min_tier` (floor) / `max_tier` (ceiling), clamped at model resolution (`sequencer._resolve_model_tier`) on every dispatch — a mechanical phase never resolves above its ceiling, a judgment gate never below its floor. Backed by a canonical tier ordering + `clamp_tier()` helper in `model_registry` (haiku<sonnet<opus). `orch validate` ERRORs on an inverted band (min_tier>max_tier) and WARNs on an unknown bound or an authored model_tier outside the band. Default (no bounds declared) is byte-identical to prior behavior. (#987)
- `coding-pipeline-standard.yaml` (→2.4.0): `existing_symbols_inventory` now declares a `sonnet` ceiling (`max_tier: sonnet`) so the mechanical inventory phase can never resolve above `sonnet`; `spec_adversary` and `review` carry an `opus` floor (`min_tier: opus`). (#987)
- Concurrent fan-out + JOIN in the loop-bearing `StateMachineSequencer` via an opt-in `parallel_group:` field on a phase (#988). When the transitions walk reaches a phase that declares a non-empty `parallel_group` (a pure fan-out node), it runs the #986 lifecycle hooks ONCE (single-threaded), dispatches the listed member phases CONCURRENTLY through the proven #102 thread-pool core (`_execute_wave_parallel` — `_phase_outputs_lock`-guarded output merge + template-level `fail_fast`/`max_parallel`, reused verbatim — no new thread model), JOINS at the pool barrier, then resolves ONE transition from the fan-out phase (all members SUCCESS → `success`; any member fail/abort, respecting `fail_fast` → the failure path, success transition NOT taken). Members are validated NON-LOOP (`max_iterations==0`) and non-nesting at build time — an `orch validate` ERROR — which makes the two shared-state races structurally impossible (the #986 `_warm_cache` race is avoided by running hooks once in the single-threaded walk; the `_current_build_iter`/`_build_phase_input` race is forbidden by the non-loop guard). The spec↔adversary and review↔fix loops, the #978 termination guard + walk-step ceiling, the #986 hook seam for normal phases, and the #987/#985 dispatch path are all preserved. No `parallel_group` declared (every bundled template, incl. `coding-pipeline-standard.yaml`) → byte-identical serial walk. Deferred follow-ups: `depends_on` auto-fan-out in the walk (#988a), nested groups (#988b), loop members, and per-group `fail_fast` overrides.
- Opt-in per-run warm build/seed CACHE + lifecycle hooks (#986) — a template may declare a top-level `lifecycle_hooks:` block (`{name → {command, invalidation: [globs]}}` + a shared fail-closed `allowed_commands` allowlist + optional `timeout_seconds`). Before each phase dispatch the `StateMachineSequencer` hashes each hook's invalidation glob-set (rooted at `config['repo_path']`, via the new `file_guard.hash_glob_set` reusing the `compute_directory_hash` digest recipe; a trailing `**` segment is normalized so it matches files recursively on Python 3.10–3.12) and runs the hook through the existing `command_executor` security model (allowlist + dangerous-pattern denylist + `shell=False` + `MAX_OUTPUT_BYTES` + timeout) ONLY on a content-hash MISS — so a `build`/`seed` runs once and is reused across the implement→acceptance_run→review→fix loop instead of cold-rebuilding every round, re-running only when its hashed inputs change. A failed or security-blocked hook ABORTS the run (`permanently_failed`; the failed hash is NOT cached) — a phase never proceeds against a stale/broken build. No `lifecycle_hooks` block → byte-identical to prior behavior (no caching, no hooks run). No new exec surface — reuses `CommandExecutor` verbatim. Deferred follow-ups: warm long-lived server with run-end teardown (#986a), `PhaseSequencer`/parallel-wave parity (#986b), and per-hook allowlists.
...
### Fixed
- **Sequencer non-termination on exhausted-route re-entry** (#978) — a loop phase that exhausted `max_iterations` and routed `EXHAUSTED` to a target that was *itself* already at/over its cap (e.g. `review`'s `failed: review` self-edge, or a mutual `a↔b` over-cap pair lacking an `exhausted:` transition) re-entered the walk WITHOUT dispatching, busy-spinning at 100% CPU with zero I/O until SIGINT. The walk now refuses to re-enter a non-dispatchable EXHAUSTED target — it falls through to the existing `MAX_ITERATIONS_EXCEEDED` abort (`aborted=True`, `error_message` naming the exhausted phase) — and a defense-in-depth walk-step ceiling backstops any other non-terminating cycle. `coding-pipeline-maintenance.yaml` (→1.2.0) gains `exhausted: fix` on its `review` and `test` loop phases for a graceful common path. Heartbeat no longer prints a `completed` count exceeding the phase total. Existing EXHAUSTED routing to a dispatchable target is unchanged.
...
- pytest no longer writes the operator's real `~/.orchestration-engine/engine.db` (#981). `default_db_path()` now honours an `ORCH_DB_PATH` env var (absolute db-file path) before the `~/.orchestration-engine/engine.db` fallback — production behaviour is byte-identical when the var is unset. A session-scoped root-conftest fixture sets `ORCH_DB_PATH` to a per-session tmp file, isolating every test and every daemon subprocess it spawns (the daemon-spawning `test_mcp_e2e` leaker that #980's HOME-only fixtures missed). HOME is deliberately untouched so the `test_ac10` `--collect-only` subprocess still finds user-site pytest on a dev box.

### Notes
- Bounds are enforced at RESOLUTION, not retry escalation: escalation is per-run (re-spawns the whole pipeline, has no failed-phase identity) and `model_override` is currently a dispatch no-op — so every real model selection flows through `_resolve_model_tier`, which is the robust seam. The `model_override` dead-write is a pre-existing gap, out of scope for #987. (#987)
...
- **CI docs-only gate no longer skips tests on mixed code+docs PRs** (#958) — the #889 gate's `docs_only` filter used `dorny/paths-filter`'s any-match semantics, so one touched `.md` file alongside code skipped the whole test/parity/e2e matrix while reporting SUCCESS. Inverted to a `non_docs` filter with `predicate-quantifier: every` over negated patterns: substantive steps run iff at least one non-docs file changed; pure docs PRs still short-circuit to SUCCESS by design. Exposure audit: every prior merge in the gate's lifetime was pure-code (full CI ran); no untested code shipped. `scripts/check_workflow.py` pins the negated globs + quantifier.
...
- **v1 backend reliability cluster — Gate 1 fixes** (#735, #753, #480; Refs #892) — Three v1-gating bugs landed together:
  ...
  - **#735 RC-1** — `templates/content-pipeline.yaml` gets per-phase `timeout_seconds`: 2400s for opus + web-search phases (`draft`, `red_team`, `final_polish`, `research`, `fact_check`), 1800s for sonnet/no-web phases (`apply_fixes`, `voice_check`). All values bounded by adversary F8: 1800 ≤ ts ≤ 3600.
  - **#735 RC-2** — New `OpenClawExecutor._check_git_committed_output()` instance method: when the gateway GC fires and `output_dir` + `output_artifact` kwargs are threaded explicitly AND the file was committed within `GC_OUTPUT_GRACE_SECONDS` (env var, default 60s), returns `(file_content, 0)` as a success signal instead of raising `RuntimeError`. CB success recorded against the SPAWN-MODEL, not git-commit metadata.
  - **#735 RC-3** — `AdaptiveRetryEngine.plan_and_execute()` git-clones original `output_dir` into `retry_output_dir` when the source is a git repo (inherits remote URL + branch); raises on clone failure (retry aborted, no orphan daemon). Non-git originals preserve existing empty-sibling-dir behaviour.
  - **#735 RC-4** — Race-free retry dedup via single-transaction `WHERE NOT EXISTS` DB check (concurrent evaluators cannot both insert a new retry row for the same `original_run_id`). Logs WARNING and returns when an active retry already exists.
