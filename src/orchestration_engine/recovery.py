...
from .schemas import ModelTier, TaskType, select_model_tier
...
class ErrorType(str, Enum):
    ...
    TRANSPORT_TIMEOUT = (
        "transport_timeout"  # HTTP socket timeout during spawn — do NOT open CB / escalate (#732)
    )
...
class TaskRetryState:
    ...
    def schedule_retry(
        self, error_type: ErrorType, error_message: str, backoff_base: int, backoff_max: int
    ) -> datetime:
        """Schedule next retry attempt."""
        backoff_seconds = min(backoff_base * (2**self.current_attempt), backoff_max)
...
class ErrorClassifier:
    ...
    def classify(
        ...
        for pattern in self.patterns:
            if pattern.matches(error_message):
                logger.debug(
                    f"Error classified as {pattern.error_type} (severity: {pattern.severity})"
                )
    ...
    def get_retry_config(self, error_type: ErrorType, task_type: TaskType) -> Dict[str, Any]:
        ...
        """
        base_config = {"max_retries": 3, "backoff_multiplier": 2.0, "escalate_model": False}

        # Error type specific adjustments
        if error_type == ErrorType.RATE_LIMIT:
            base_config.update({"max_retries": 8, "backoff_multiplier": 2.5})
        elif error_type == ErrorType.QUALITY:
            base_config.update({"max_retries": 3, "escalate_model": True})
        elif error_type == ErrorType.RESOURCE:
            base_config.update({"max_retries": 5, "backoff_multiplier": 3.0})
        elif error_type == ErrorType.PERMANENT:
            base_config.update({"max_retries": 0})
...
class RecoveryManager:
    ...
    def _init_tables(self) -> None:
        ...
        """)
        self.db.execute("CREATE INDEX IF NOT EXISTS idx_retry_task ON retry_attempts(task_id)")
        self.db.execute(
        ...
        self.db.execute("CREATE INDEX IF NOT EXISTS idx_error_type ON error_patterns(error_type)")
    ...
    def handle_task_failure(
        ...
        retry_at = retry_state.schedule_retry(
            error_type, error_message, self.config.retry.backoff_base, self.config.retry.backoff_max
        )
        ...
        logger.info(
            f"Task {task_id} scheduled for retry {retry_state.current_attempt} at {retry_at} "
    ...
    def handle_task_success(
        self, task_id: str, task_type: TaskType, model_tier: str = None
    ) -> None:
    ...
    def _get_retry_state(
        self, task_id: str, task_type: TaskType, original_model: str
    ) -> TaskRetryState:
    ...
    def _record_error_pattern(
        self, error_message: str, error_type: ErrorType, task_type: TaskType, model_tier: str = None
    ) -> None:
    ...
    def get_retry_queue(self) -> List[Dict[str, Any]]:
        ...
        return [{"task_id": row["task_id"], "scheduled_at": row["next_retry"]} for row in results]
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "retry_statistics": (
                retry_stats[0]
                if retry_stats
                else {"total_retries": 0, "successful_retries": 0, "avg_backoff": 0}
            ),
        }
