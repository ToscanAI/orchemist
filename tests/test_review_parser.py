...
class TestVerdictParsing(unittest.TestCase):
    ...
    def test_verdict_found_after_five_preamble_lines(self) -> None:
        """Cascade finds verdict even when 5+ non-blank lines precede it (Layer 1)."""
        text = (
            "Preamble line one\n"
            "Preamble line two\n"
            "Preamble line three\n"
...
# ===========================================================================
# TestExtractVerdictCascade
# ===========================================================================


class TestExtractVerdictCascade(unittest.TestCase):
    """Tests for the extract_verdict() 4-layer cascade function."""

    # ── Layer 0: Quick first-5-lines scan ─────────────────────────────────────

    def test_layer0_approve_on_first_line(self) -> None:
        """Layer 0 finds APPROVE on line 1."""
        self.assertEqual(extract_verdict("APPROVE\n"), "APPROVE")

    def test_layer0_request_changes_on_first_line(self) -> None:
        """Layer 0 finds REQUEST_CHANGES on line 1."""
        self.assertEqual(extract_verdict("REQUEST_CHANGES\n"), "REQUEST_CHANGES")

    def test_layer0_verdict_after_blank_lines(self) -> None:
        """Layer 0 skips blank lines and finds verdict within first 5 non-blank."""
        text = "\n\n\nAPPROVE\n"
        self.assertEqual(extract_verdict(text), "APPROVE")

    def test_layer0_verdict_with_trailing_comment(self) -> None:
        """Layer 0 matches on first word even with trailing text on same line."""
        self.assertEqual(extract_verdict("APPROVE — looks great\n"), "APPROVE")

    # ── Layer 1: Full-text scan ────────────────────────────────────────────────

    def test_layer1_verdict_buried_deep(self) -> None:
        """Layer 1 finds verdict buried beyond the 5-line window."""
        lines = [f"Preamble line {i}\n" for i in range(10)]
        lines.append("APPROVE\n")
        self.assertEqual(extract_verdict("".join(lines)), "APPROVE")

    def test_layer1_verdict_in_conclusion_paragraph(self) -> None:
        """Layer 1 finds 'In conclusion... APPROVE' style placement."""
        text = (
            "Overall analysis:\n"
            ...
            "In conclusion, APPROVE\n"
        )
        self.assertEqual(extract_verdict(text), "APPROVE")

    def test_layer1_skips_code_block_verdict(self) -> None:
        """Layer 1 does not extract a verdict token inside a fenced code block.

        The code block is preceded by enough non-verdict preamble lines to
        exhaust Layer 0's 5-line scan window, forcing Layer 1 to evaluate the
        block.  Layer 1 skips the fenced APPROVE and finds the real verdict.
        """
        preamble = "\n".join(f"Preamble line {i}." for i in range(6))
        text = (
            preamble + "\n"
            "```\n"
            "APPROVE\n"
            "```\n"
            "But the actual verdict is REQUEST_CHANGES\n"
        )
        result = extract_verdict(text)
        self.assertEqual(result, "REQUEST_CHANGES")

    def test_layer1_skips_negative_prefix_would_not(self) -> None:
        """Layer 1 skips 'would not APPROVE' lines (negative prefix)."""
        text = "\n".join([f"Line {i}" for i in range(10)] + [
            "I would not APPROVE this as-is, but here are my thoughts.",
            "After reflection: REQUEST_CHANGES",
        ])
        self.assertEqual(extract_verdict(text), "REQUEST_CHANGES")

    def test_layer1_skips_negative_prefix_not(self) -> None:
        """Layer 1 skips 'not APPROVE' occurrences."""
        text = "\n".join([f"Line {i}" for i in range(10)] + [
            "This is not APPROVE worthy.",
            "My verdict: APPROVE",
        ])
        self.assertEqual(extract_verdict(text), "APPROVE")
    ...
    def test_layer2_verdict_at_end_of_long_doc(self) -> None:
        """Layer 2 finds verdict in the tail of a long document."""
        long_prose = "\n".join(f"Analysis point {i}." for i in range(50))
        text = long_prose + "\n\nFinal verdict: APPROVE\n"
        self.assertEqual(extract_verdict(text), "APPROVE")

    def test_layer2_request_changes_at_very_end(self) -> None:
        """Layer 2 finds REQUEST_CHANGES at the bottom of a long review."""
        long_prose = "\n".join(f"Detailed comment {i}." for i in range(40))
    ...
    def test_returns_none_when_no_verdict(self) -> None:
        """Returns None when text contains no verdict token."""
        self.assertIsNone(extract_verdict("No verdict here at all.\n"))

    def test_returns_none_for_empty_string(self) -> None:
        """Returns None for empty input."""
        self.assertIsNone(extract_verdict(""))

    def test_returns_none_for_only_blank_lines(self) -> None:
        """Returns None for whitespace-only input."""
        self.assertIsNone(extract_verdict("\n\n\n"))

    def test_non_string_coerced(self) -> None:
        """Non-string inputs are coerced and don't raise."""
        try:
            result = extract_verdict(None)  # type: ignore[arg-type]
            self.assertIsNone(result)
        except Exception as exc:
            self.fail(f"extract_verdict raised unexpectedly: {exc!r}")

    def test_both_verdicts_first_wins(self) -> None:
        """When both verdicts appear, the first found wins (Layer 0)."""
        text = "APPROVE\nREQUEST_CHANGES\n"
        self.assertEqual(extract_verdict(text), "APPROVE")

    def test_exported_in_dunder_all(self) -> None:
        """extract_verdict is listed in __all__."""
        import orchestration_engine.review_parser as _mod
        self.assertIn("extract_verdict", _mod.__all__)


# ===========================================================================
# TestSmartFullTextScan
# ===========================================================================


class TestSmartFullTextScan(unittest.TestCase):
    """Edge-case tests for Layer 1: _smart_full_text_scan."""

    def test_finds_approve_anywhere_in_text(self) -> None:
        """APPROVE found on any line, not just the first 5."""
        lines = [f"line {i}" for i in range(20)] + ["APPROVE"]
        result = extract_verdict("\n".join(lines))
        self.assertEqual(result, "APPROVE")

    def test_skips_nested_code_blocks(self) -> None:
        """Verdicts inside ``` fences are ignored.

        Six preamble lines push the code fence past Layer 0's window so that
        Layer 1 is responsible for evaluating it.
        """
        preamble = "\n".join(f"Analysis note {i}." for i in range(6))
        text = (
            preamble + "\n"
            "```\n"
            "REQUEST_CHANGES\n"
            "```\n"
            "My actual verdict after deep analysis is APPROVE\n"
        )
        self.assertEqual(extract_verdict(text), "APPROVE")

    def test_multiple_code_blocks_toggle_correctly(self) -> None:
        """Two code blocks: verdict in first (skipped), verdict after second (found).

        Six preamble lines push the first code block past Layer 0's window.
        """
        preamble = "\n".join(f"Review comment {i}." for i in range(6))
        text = (
            preamble + "\n"
            "```\n"
            "APPROVE\n"
            "```\n"
            "Prose outside first block.\n"
            "```\n"
            "More code here.\n"
            "```\n"
            "REQUEST_CHANGES\n"
    ...
    def test_negative_prefix_dont(self) -> None:
        """Lines with 'don't APPROVE' are skipped."""
        text = "\n".join([f"Preamble {i}" for i in range(6)] + [
            "I don't APPROVE of this pattern, but let me check more.",
            "Verdict: REQUEST_CHANGES",
        ])
        self.assertEqual(extract_verdict(text), "REQUEST_CHANGES")

    def test_negative_prefix_avoid(self) -> None:
        """Lines with 'avoid APPROVE' are skipped."""
        text = "\n".join([f"Preamble {i}" for i in range(6)] + [
            "We should avoid APPROVE without proper review.",
            "My decision: APPROVE",
        ])
        self.assertEqual(extract_verdict(text), "APPROVE")

    def test_verdict_in_markdown_bold_not_filtered(self) -> None:
        """A verdict in **bold** markdown is still found."""
        text = "\n".join([f"Line {i}" for i in range(6)] + [
            "**APPROVE** — the implementation is solid.",
        ])
        self.assertEqual(extract_verdict(text), "APPROVE")

    def test_case_insensitive_verdict_detection(self) -> None:
        """Lowercase 'approve' in prose is still found by Layer 1."""
        text = "\n".join([f"Preamble {i}" for i in range(6)] + [
...
# ===========================================================================
# TestTailWeightedScan
# ===========================================================================


class TestTailWeightedScan(unittest.TestCase):
    """Edge-case tests for Layer 2: _tail_weighted_scan."""

    def _long_preamble(self, n: int = 25) -> str:
        """Generate n prose lines that contain no verdict tokens."""
        return "\n".join(f"Analysis note number {i}." for i in range(n))

    def test_finds_verdict_in_last_line(self) -> None:
        """Verdict on the very last line is found by tail scan."""
        text = self._long_preamble(25) + "\nAPPROVE"
        self.assertEqual(extract_verdict(text), "APPROVE")

    def test_finds_request_changes_in_last_line(self) -> None:
        text = self._long_preamble(25) + "\nREQUEST_CHANGES"
        self.assertEqual(extract_verdict(text), "REQUEST_CHANGES")

    def test_tail_scan_handles_short_document(self) -> None:
        """Tail scan works correctly on documents shorter than 20 lines."""
        text = "APPROVE"
        self.assertEqual(extract_verdict(text), "APPROVE")

    def test_tail_scan_negative_prefix_still_skipped(self) -> None:
        """Tail scan also respects negative prefixes."""
        text = self._long_preamble(25) + "\nI wouldn't APPROVE without more tests.\nREQUEST_CHANGES"
        self.assertEqual(extract_verdict(text), "REQUEST_CHANGES")

    def test_first_verdict_in_long_doc_wins(self) -> None:
        """When multiple verdicts exist in a long document, Layer 1 finds the first.

        Layer 1 (full-text top-to-bottom scan) fires before Layer 2 and will
        return the *first* unambiguous verdict token encountered.  Layer 2 is
        only invoked when Layer 1 finds nothing at all.
        """
        text = (
            self._long_preamble(25)
            + "\nAPPROVE (initial thought)\n"
            + "\n".join(f"Reconsidering point {i}." for i in range(5))
            + "\nREQUEST_CHANGES"
        )
        # Layer 1 runs top-to-bottom: APPROVE appears first → wins
        self.assertEqual(extract_verdict(text), "APPROVE")
